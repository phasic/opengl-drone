...
void keyboard(unsigned char key, int x, int y)
{
	...
	case '5': zeroSwitch = !zeroSwitch; lightControl(0); break;
	case '6': oneSwitch = !oneSwitch; lightControl(0); break;
	case '7': twoSwitch = !twoSwitch; lightControl(0); break;
	case '8': droneSwitch = !droneSwitch; lightControl(0); break;
	case 'r': scale[0] = 1.0, scale[1] = 1.0, scale[2] = 1.0,			/*reset*/
		oog[0] = 75.0, oog[1] = 75.0, oog[2] = 75.0,
		camAngleX = -135, camAngleY = -45;	 break;
	case 'a': camAngleX -= 1;			 break;								/*rotate*/
	case 'd': camAngleX += 1;		     break;
	case 's': camAngleY += 1;            break;
	case 'w': camAngleY -= 1;            break;
	case 'h': changeHeight = 2;			 break;
	case 'H': changeHeight = 1;			 break;
	case 'l': masterSwitch = !masterSwitch; lightControl(1); break;

	case 'q': scale[0] *= 1.1; scale[1] *= 1.1; scale[2] *= 1.1; break;	/*scale*/
	case 'e': scale[0] /= 1.1; scale[1] /= 1.1; scale[2] /= 1.1; break;
	
	case 'v': show = !show; break;
	case 'f': flat = !flat; break;
	}
...
void animPropeller(int delta)
{
	...
	if (anim)
	{
		...
		if (animHoek > 360.0) animHoek -= 360.0;
		if (animHoek < -360.0) animHoek += 360.0;
	}
...
void animFlyRotation(int delta)
{
	if (fly)
	{
		...
		if (animFlyHoek > 360.0) animFlyHoek -= 360.0;
		if (animFlyHoek < -360.0) animFlyHoek += 360.0;
	}
	glutSwapBuffers();
	glutPostRedisplay();
...
void materialDrone(int object)
{
	...
	switch (object)
	{
	case PROPELLER:	glEnable(GL_COLOR_MATERIAL);
	...
	case FRAME:	glEnable(GL_COLOR_MATERIAL);
		glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);
	...
	case BODY:	glEnable(GL_COLOR_MATERIAL);
		glColorMaterial(GL_FRONT_AND_BACK, GL_AMBIENT_AND_DIFFUSE);
...
int main(int argc, char * argv[])
{
	if (argc > 1)
	{
		...
		if (typ == 'F')
		{
			xmin = -1.0; xmax = 2.0; ymin = -1.0; ymax = 1.5;
		}
	}
	else
